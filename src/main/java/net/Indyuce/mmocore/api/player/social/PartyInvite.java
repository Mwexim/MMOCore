...
public class PartyInvite extends Request {
	...
	public void accept() {
		...
		party.getMembers().forEach(member -> MMOCore.plugin.configuration.getSimpleMessage("party-joined-other", "player", target.getPlayer().getName()).send(member.getPlayer()));
		MMOCore.plugin.configuration.getSimpleMessage("party-joined", "owner", party.getOwner().getPlayer().getName()).send(target.getPlayer());
