...
public class GuildInvite extends Request {
	...
	public void accept() {
		...
		guild.getMembers().forEach(member -> MMOCore.plugin.configuration.getSimpleMessage("guild-joined-other", "player", target.getPlayer().getName()).send(Bukkit.getPlayer(member)));
		MMOCore.plugin.configuration.getSimpleMessage("guild-joined", "owner", Bukkit.getPlayer(guild.getOwner()).getName()).send(target.getPlayer());
